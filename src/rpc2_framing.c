...
static frame_decode_result_t content_length_decode(framing_st *f, char *buf,
                                                   size_t buf_len,
                                                   size_t *msg_offset,
                                                   size_t *msg_len) {
  ...
  for (;;) {
    if (fl->in_header) {
      ...
      if (sscanf(content_len_str, "Content-Length: %d", &fl->content_length) !=
              1 ||
          fl->content_length < 0) {
      ...
      fprintf(stderr,
              ...
              fl->content_length, fl->body_offset, buf_len);
    }
...
static char *content_length_encode(framing_st *f, char const *body,
                                   size_t body_len, size_t *framed_len) {
  (void)f;
...
static void content_length_destroy(framing_st *f) { free(f); }
...
static frame_decode_result_t newline_decode(framing_st *f, char *buf,
                                            size_t buf_len, size_t *msg_offset,
                                            size_t *msg_len) {
  (void)f;
...
static char *newline_encode(framing_st *f, char const *body, size_t body_len,
                            size_t *framed_len) {
  (void)f;
...
static void newline_destroy(framing_st *f) { free(f); }
